...
export default function AlugarPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!item) {
      return setError("Item não carregado.");
    }
    ...
    try {
      ...
      // Calcular endDate somando durationDays a startDate
      const start = new Date(startDate);
      const end = new Date(start);
      end.setDate(start.getDate() + durationDays);

      // Calcular totalPrice
      const totalPrice = item.pricePerDay * durationDays;
      ...
      const res = await fetch("/api/orders", {
        ...
        body: JSON.stringify({
          ...
          totalPrice,
          status: "pending", // opcional, backend já define padrão
        }),
      });
  ...
  return (
    <div className="min-h-screen bg-[#417FF2] py-12 px-4 flex items-start justify-center">
      ...
      >
        ...
        <div>
          <label className="block text-[#417FF2] font-medium mb-1">
            Data de Início:
          </label>
          <input
            type="date"
