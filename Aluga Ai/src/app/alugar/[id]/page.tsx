...
export default function AlugarPage() {
	...
	const calculateTotalPrice = useMemo(() => {
		...
		// Validação básica: data de término deve ser posterior à de início
		if (endDateTime <= startDateTime) return 0;

		const durationMs = endDateTime.getTime() - startDateTime.getTime();
		let durationHours = durationMs / (1000 * 60 * 60);

		// Define a tarifa por hora. Se o item tiver pricePerHour, usa-o.
		// Caso contrário, calcula com base no pricePerDay.
		// ATENÇÃO: Adapte esta linha se seu modelo Item tiver 'pricePerHour'
		const pricePerHour = (item as any).pricePerHour || item.pricePerDay / 24;

		// Arredonda para cima para a hora mais próxima (ex: 2.1 horas se tornam 3 horas)
		// Isso garante que mesmo uma fração de hora seja cobrada como uma hora inteira.
		const totalHoursToCharge = Math.ceil(durationHours);

		// Lógica de cobrança: no mínimo 1 hora, depois por hora
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			if (!res.ok)
				return setError(
					data.error || "Ocorreu um erro ao criar o pedido. Tente novamente."
				);
	...
	const now = new Date();
	// Ajusta a data e hora para o fuso horário local e formata para o input
	const offset = now.getTimezoneOffset() * 60000; // offset em ms
	const localNow = new Date(now.getTime() - offset);
	const minDateTimeLocal = localNow.toISOString().slice(0, 16); // "YYYY-MM-DDTHH:mm"

	// Funções para definir os valores de data e hora separadamente
				...
				<div className="bg-white rounded-3xl shadow-2xl p-0 w-full max-w-xl overflow-hidden flex flex-col md:flex-row animate-fade-in-down">
					...
					<div className="p-8 md:w-1/2 space-y-6">
						...
						<form onSubmit={handleSubmit} className="space-y-6">
							{/* Data e Hora de Início (usando datetime-local para simplificar o input) */}
							<div>
								<label
								...
								<input
									...
									min={minDateTimeLocal} // Impede seleção de data/hora passadas
							...
							{/* Data e Hora de Término (usando datetime-local) */}
							<div>
								<label
									htmlFor="endDateTime"
									className="block text-[#417FF2] font-semibold mb-2">
								...
								<input
									...
									// Define o mínimo como a data/hora de início selecionada ou a hora atual se nada foi selecionado ainda
									min={
										startDate && startTime
							...
							{calculateTotalPrice > 0 && item && (
								<div className="bg-[#eaf1fb] p-4 rounded-lg text-center border border-[#c1d1ee]">
									...
									{/* Mensagem dinâmica sobre a base do cálculo */}
